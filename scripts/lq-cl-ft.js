...
(() => {
    ...
    let h = document.querySelector(".box-title")?.textContent.trim() || "",
        name = "";
    if (h) name = (location.href.includes("lesson-step1") ? h.match(/^(.+?)\s+'s\s+Lesson Details$/) : h.match(/^(.+?)\s+'s\s+Additional Lesson Details$/) || [])[1] || "";
    ...
    toast.id = "toast";
    site.appendChild(toast);
    GM_addStyle(`html,body{margin:0;font-family:Play}#site{width:100%;padding:12px;min-height:100vh;transition:width .35s ease}#panel{position:fixed;right:0;top:0;width:20vw;height:100vh;background:#020617;color:#e5e7eb;padding:14px;display:flex;flex-direction:column;transition:transform .28s ease;z-index:9999}#panel.closed{transform:translateX(100%)}#close{position:absolute;top:8px;right:8px;cursor:pointer;opacity:.4}#header{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}#name,#timer{font-size:12px;color:#93c5fd}#timer i{margin-right:6px}#switcher{display:flex;gap:6px;margin-bottom:8px}#switcher button{flex:1;background:transparent;border:1px solid #0b1220;color:#93c5fd;padding:6px;border-radius:8px;cursor:pointer;opacity:.6}#switcher button.active{background:rgba(250,204,21,0.06);color:#facc15;opacity:1;border-color:rgba(250,204,21,0.16)}#featureArea{flex:1;overflow:auto}#perfContainer{display:flex;flex-direction:column;height:100%;justify-content:space-between}.l{font-size:11px;opacity:.6;letter-spacing:.1em}#box{flex:1;text-align:center;margin-top:0;display:flex;flex-direction:column;justify-content:center}#score{position: relative; font-size: 90px; font-weight: 800; color: #facc15;}#lists{margin-top:auto;display:flex;width:100%;gap:8px}#lists>div{width:50%;flex:0 0 47%}#pos,#neg{width:100%}h3{text-align:center;font-size:12px;opacity:.7}.item{margin:4px 0;padding:6px;border-radius:8px;font-size:12px;font-weight:700;text-align:center;cursor:pointer}#toast{position:fixed;top:40px;left:0;width:100%;display:flex;justify-content:center;pointer-events:none;z-index:10000}#toast::before{content:attr(data-text);padding:18px 50px;border-radius:28px;font-size:30px;font-weight:800;color:#fff;background:var(--bg);opacity:0;transform:translateY(-20px);transition:.4s}#toast.show::before{opacity:1;transform:none}#mini{position:fixed;top:10px;right:10px;width:34px;height:34px;border-radius:50%;background:#020617;color:#93c5fd;display:none;align-items:center;justify-content:center;cursor:pointer;z-index:9999;box-shadow:0 0 12px rgba(0,0,0,.6)}#featContent{height: 90%; margin-top:6px; display: flex; flex-direction: column;}.lq-task{background:#071025;border:1px solid #122032;border-radius:10px;padding:10px;display:flex;align-items:center;gap:10px;margin-bottom:6px}.lq-task span{flex:1;transition:color .35s linear}.lq-task input{flex:1;background:transparent;border:none;outline:none;color:#facc15}.lq-task i{cursor:pointer;opacity:.6}.lq-bars{flex:1;display:flex;align-items:flex-end;gap:4px;justify-content:flex-start;overflow-x:auto;height:200px;position:relative;width:100%}.lq-bars .bar{width:12px;border-radius:3px;background:linear-gradient(180deg,#facc15,#f59e0b);display:inline-block;position:relative}.lq-bars .bar span{position:absolute;top:-18px;left:50%;transform:translateX(-50%) rotate(-90deg);font-size:10px;color:#facc15;font-weight:700}.lq-labels{display:flex;gap:4px;margin-top:4px;justify-content:flex-start;align-items:flex-start}.lq-labels div{writing-mode:vertical-lr;text-align:center;font-size:10px;color:#9ca3af;width:12px}.lq-stats{display:flex;flex-direction:column;gap:6px;margin-bottom:6px}.lq-meta{display:flex;justify-content:space-between;font-size:12px;color:#93c5fd;opacity:.9}.pswp{position:fixed!important;left:0!important;top:0!important;width:80vw!important;height:100vh!important;z-index:9998!important}.pswp__bg{width:80vw!important;background:rgba(2,6,23,0.92)!important}.pswp__scroll-wrap,.pswp__container,.pswp__item{width:80vw!important}.pswp__ui{right:auto!important}.pswp__button--fs{display:none!important}`);
    const open = () => {
        panel.classList.remove("closed");
        mini.style.display = "none";
        pref.panelOpen = true;
        S("listOfStudents", students);
        site.style.width = 'calc(100% - 20vw)'
    };
    const close = () => {
    ...
    const pushOrUpdateHistory = () => {
        ...
        if (hlist.length > 50) feats[hid].history = hlist.slice(-50);
        S("lq_features", feats)
    };
    ...
    const renderResults = () => {
        ...
        barsWrap.style.width = (panel.classList.contains("closed") ? "100%" : "calc(100% - 20px)");
    ...
    const setFeature = f => {
        ...
        const featContent = panel.querySelector("#featContent");
        if (f === "performance") {
    ...
    panel.querySelectorAll("#switcher button").forEach(b => {
        b.onclick = () => {
            pref.feature = b.dataset.f; // save only on click
            S("listOfStudents", students);
